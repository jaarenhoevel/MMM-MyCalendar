...
| `joiningWord`                | Word to display between the day/date and the time of the event (e.g.: Tuesday `at` 1:15 PM). If you do not want one, you need to explicitly specify an empty string (i.e.: `""`)<br> **Default value:** `at`
| `endDateSeperator`           | String to display between the start and end time of the event (e.g.: Tuesday at 1:15 PM` - ` Friday at 10:00 AM). If you do not want a dash, you need to explicitly specify a single space (i.e.: `" "`)<br> **Default value:** ` - `
| `displayEndDate`             | If set to true, the end date of all events will be displayed as well<br> **Default value:** `true`
